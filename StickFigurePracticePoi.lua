...
local lassoassoasso = function(core, part)
	local lasso = Instance.new("SelectionPartLasso")
	lasso.Parent = core
	lasso.Color = color
	coroutine.resume(coroutine.create(function() local head = Character:FindFirstChild("Head") while true do lasso.Color = BrickColor.Red() head.BrickColor = lasso.Color wait(4) lasso.Color = BrickColor.Green() head.BrickColor = lasso.Color wait(4) lasso.Color = BrickColor.Blue() head.BrickColor = lasso.Color wait(4) end end))
	lasso.Humanoid = (owner.Character.Humanoid or part.Parent:FindFirstChildWhichIsA("Humanoid") or Instance.new("Humanoid",part.Parent))
	lasso.Part = part
	--print(core,part, lasso.Humanoid)
	return lasso
end
...
local stickFigures = function()	for i,v in pairs(game.Players:GetChildren()) do	pcall(function() stickFigure(v.Character) end) end end
...
function buildPoi(arm, col, col_rope, col_knob)
	...
	local _poi = Model('Poi', Character or workspace)
...
function manipulateCharacter(_Character)
	...
	local ArmRotation = function(Arm)
		coroutine.resume(coroutine.create(function()
			...
			for i = 1,16 do wait() w.C1 = w.C1 * CFrame.Angles(angl,0,0) end
			while true do 
				angl = -angl
				for i = 1,32 do wait() w.C1 = w.C1 * CFrame.Angles(angl,0,0) end
				for i = 1,32 do wait() w.C1 = w.C1 * CFrame.Angles(0,0.05,0) * CFrame.Angles(0,0,-angl) end
			end
		end))
	end
